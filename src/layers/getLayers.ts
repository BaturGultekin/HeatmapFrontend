...
export const getLayers = ({
  ...
  if (dataState && heatmapState) {
    ...
    // Temporary compatibility with the older polygon heatmap renderer.
    // The old renderer expects an explicit array of visible cell indices.
    const visibleIndices = Array.from(
      { length: heatmapState.cellData.rowIndices.length },
      (_, index) => index
    ).filter((index) => {
      if (!visibleBounds) return true;

      const rowIndex = heatmapState.cellData.rowIndices[index];
      const colIndex = heatmapState.cellData.colIndices[index];

      return (
        rowIndex >= visibleBounds.startRow &&
        rowIndex <= visibleBounds.endRow &&
        colIndex >= visibleBounds.startCol &&
        colIndex <= visibleBounds.endCol
      );
    });

    const heatmapGridLayer = getHeatmapGridLayer(
      heatmapState,
      opacityVal,
      onClick?.heatmapCell,
      visibleIndices,
